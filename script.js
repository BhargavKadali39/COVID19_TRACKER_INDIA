...
var dist = 0;
loadNames();
// console.log(jsds['Andhra Pradesh']['districtData']['Vizianagaram']);
//const names = null;
// loadNames();
// let jsjs = getData(url1);
...
async function loadNames() {
  ...
  // console.log(names['Andhra Pradesh']['districtData']['Vizianagaram']);
  GFG_Fun(states);
}
...
function GFG_Fun(elmts) {
  ...
  // add the upper text to ask to select dist
}
...
//nevercalled  elmts = state name
async function GFG_Fun2(elmts) {
  var response = await fetch(url1);
  const names = await response.json();
  // var ttt = elmts['Andhra Pradesh']['districtData']['Vizianagaram']
  // console.log(ttt);
